...
const getLast10FilteredSeries = async (req) => {
  const queryFilters = req.data
    ...
    .join(" ");
  const queryParams = `SELECT s.id AS 'id',s.seasons_count AS 'seasons_count', s.name AS 'name', s.description AS 'description', s.image_cover AS 'image_cover', GROUP_CONCAT(DISTINCT ap.action_place SEPARATOR ', ') AS 'action_place', MIN(sat.action_time_start) AS 'action_time_start', MAX(sat.action_time_end) AS 'action_time_end', MIN(py.production_year_start) AS 'production_year_start', MAX(py.production_year_end) AS 'production_year_end', GROUP_CONCAT(DISTINCT sc.serie_category SEPARATOR ', ') AS 'category', sr.serie_rating AS 'series_rating' FROM series s LEFT JOIN series_action_place ap ON s.id = ap.serie_id LEFT JOIN series_categories sc ON s.id = sc.serie_id LEFT JOIN series_rating sr ON s.id = sr.serie_id LEFT JOIN series_action_time sat ON s.id = sat.serie_id LEFT JOIN series_production_year py ON s.id = py.serie_id WHERE s.ready_to_publish=1 AND ${queryFilters} GROUP BY s.id, s.name, s.description, s.image_cover, sr.serie_rating order by s.id desc;
`;
  const rows = await db.query(queryParams);

  return rows;
};
const getNext5FilteredSeries = async (req) => {
  const queryFilters = req.data
    .map((item, index) => {
      //check if there is only one filter (no need to put and after each filter)
      if (req.data.length === 1) {
